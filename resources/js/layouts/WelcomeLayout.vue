...
onMounted(() => {
    if (page.props.auth.user) {
        const authMenu = [
            {
                title: 'Sikat Gigi',
                link: '/welcome/brushing-teeth'
            },
            {
                title: 'Pre Test',
                link: '/welcome/pre-test'
            },
            {
                title: 'Post Test',
                link: '/welcome/post-test'
            },
            {
                title: 'Konsultasi',
                link: '/welcome/consultation'
            },
        ]

        authMenu.map((item) => {
            menuHeader.value.push(item)
        })
            ...
            <header class="absolute inset-x-0 top-0 z-50">
                <nav class="flex items-center justify-between p-6 lg:px-8" aria-label="Global">
                    ...
                    <div class="hidden lg:flex lg:gap-x-12">
                        <button @click="handleClick(menu)" type="button"
                            class="text-sm font-semibold leading-6 text-gray-900 cursor-pointer"
                            v-for="menu in menuHeader" :key="menu.title">{{ menu.title }}</button>
                    </div>
                    <div class="hidden lg:flex lg:flex-1 lg:justify-end">
                        <ProfileDropdown v-if="page.props.auth.user" />
                        <button v-else type="button" @click="toLogin"
